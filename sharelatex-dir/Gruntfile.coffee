...
SERVICES_FCC = [{
	name: "chat"
	repo: "https://github.com/fcarlier/chat-sharelatex.git"
}]

SERVICES_ALL = SERVICES.concat SERVICES_FCC
...
module.exports = (grunt) ->
	...
	grunt.initConfig
		...
		concurrent:
			all:
				tasks: ("run:#{service.name}" for service in SERVICES_ALL)
				options:
					limit: SERVICES_ALL.length
				...
				options:
					...
					groups:
						"Run tasks": [
							...
							"run:all"
							"default"
						].concat ("run:#{service.name}" for service in SERVICES_ALL)
						"Misc": [
							"help"
						]
	...
	grunt.registerTask "run:chat", "Run the ShareLaTeX chat service", ["bunyan", "execute:chat"]
